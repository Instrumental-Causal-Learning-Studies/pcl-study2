...
function mapIntention(sentence){
  if(sentence == "How likely is it that Red Brock intentionally opened the door?"){
    return "intention1"
  } else if(sentence ==  "Are Red Brock's actions intentional?"){
    return "intention2"
  } else if(sentence == "How likely is it that Red Brock knew how to open the door?"){
    return "knowledge"
  } else if(sentence == "Was Red Brock aiming to open the door?"){
    return "goal"
  } else if(sentence == "Was Red Brock aiming to clean the room?"){
    return "goalClean"
  } else if(sentence == "How likely is it that Blue Brock intentionally opened the door?"){
    return "intention1B"
  }else if(sentence ==  "Are Blue Brock's actions intentional?"){
    return "intention2B"
  } else if(sentence == "How likely is it that Blue Brock knew how to open the door?"){
    return "knowledgeB"
  } else if(sentence == "Was Blue Brock aiming to open the door?"){
    return "goalB"
  } else if(sentence == "Was Blue Brock aiming to clean the room?"){
    return "goalCleanB"
  }
...
slides.intentionH = slide({
  ...
  present : randomSort([
    {sentence: "How likely is it that Red Brock intentionally opened the door?"}, //intention
    {sentence: "How likely is it that Red Brock knew how to open the door?"}, //knowledge
    {sentence: "Was Red Brock aiming to open the door?"}, //goal
    {sentence: "Was Red Brock aiming to clean the room?"}, //goalClean
    {sentence: "Are Red Brock's actions intentional?"} //intention
  ]),
...
slides.intentionB = slide({
  ...
  /* trial information for this block
   (the variable 'stim' will change between each of these values,
    and for each of these, present_handle will be run.) */
  present : randomSort([
    {sentence: "How likely is it that Blue Brock intentionally opened the door?"}, //intention
    {sentence: "How likely is it that Blue Brock knew how to open the door?"}, //knowledge
    {sentence: "Was Blue Brock aiming to open the door?"}, //goal
    {sentence: "Was Blue Brock aiming to clean the room?"}, //goalClean
    {sentence: "Are Blue Brock's actions intentional?"} //intention
  ]),
...
slides.attention2B =  slide({ //check this!!!
  ...
  button : function(){
    ...
    if($('input[name=blueB]:checked').length == 0|$('input[name=pinkB]:checked').length == 0|$('input[name=orangeB]:checked').length == 0) {
      $(".err").show();
    } else {
  ...
  log_responses : function() {
    append(exp.data,
      {
        "attention2BRT": this.RT,
        "attentionBlueB" : $('input[name="blueB"]:checked').val(),
        "attentionPinkB" : $('input[name="pinkB"]:checked').val(),
        "attentionOrangeB" : $('input[name="orangeB"]:checked').val()
      }
      )
...
function sendDataToServer(data) {
	$.ajax({
    url: 'https://script.google.com/macros/s/AKfycbzqziDyzeh-9LG2430d9ysUcOR439t7RlFqWeOlfLco4cLFeWqme3C-TavcKkQDkqWzaQ/exec',
		type: 'post',
    data: data,
    complete: function(){
      window.location.replace("https://app.prolific.co/submissions/complete?cc=7C8B63F1") // redirect to prolific
    } 
	});
}
